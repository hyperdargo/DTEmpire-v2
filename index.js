...
process.on('uncaughtException', (error) => {
    ...
    errorHandler.logCrash(error, 'UNCAUGHT_EXCEPTION');
});

// Handle unhandled promise rejections
process.on('unhandledRejection', (reason, promise) => {
    const timestamp = new Date().toISOString();
    const errorMessage = `[${timestamp}] UNHANDLED REJECTION at ${promise}: ${reason}`;
    console.error('🔴 ' + errorMessage);
    
    const error = reason instanceof Error ? reason : new Error(String(reason));
    errorHandler.logCrash(error, 'UNHANDLED_REJECTION');
});
...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        errorHandler.logEventError(error, 'messageCreate', {
...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        errorHandler.logEventError(error, 'interactionCreate', {
            userId: interaction.user?.id,
            guildId: interaction.guild?.id,
            customId: interaction.customId,
            type: interaction.type
        });
        console.error('❌ Error in interactionCreate event:', error.message);
...
client.on('error', (error) => {
    ...
    errorHandler.logError(error, 'DISCORD_CLIENT_ERROR');
});

// Handle shard errors
client.on('shardError', (error, shardId) => {
    console.error(`🔴 Shard ${shardId} Error:`, error);
    errorHandler.logError(error, `SHARD_ERROR_${shardId}`);
});
...
async function startBot() {
    try {
        console.log('🚀 Starting DTEmpire v2.8.0...');
